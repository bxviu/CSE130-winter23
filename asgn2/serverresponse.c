...
int send_response(Response *res, int sock, int infile) {
    ...
    char str_res[100];
    snprintf(str_res, sizeof(str_res), "%s %s %s\r\n%s: ", res->sts_l->version, res->sts_l->code,
        res->sts_l->phrase, res->head_f->key);
    int read_bytes = 0;
    //printf("|%s|\n", res->head_f->value);
    if (!strcmp(res->head_f->value, "?")) {
        ...
        //printf("|%s|\n", str_res);
        write_all(sock, str_res, strlen(str_res));
        read_bytes = pass_bytes(infile, sock, g);
        //close(infile);
        //int r = close(infile);
        //fprintf(stderr, "%d\n", r);
        //printf("%d %d\n", r, infile);
...
Response *create_response(enum StatusCode statcode, int infile) {
    Status_Line *sl
        = malloc(sizeof(Status_Line)); //{ .version = "HTTP/1.1", .code = "200", .phrase = "OK" };
